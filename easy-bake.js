...
(function() {
  var RESERVED_SETS, RUNNERS_ROOT, TEST_DEFAULT_TIMEOUT, cake, eb, ebc, fs, globber, path, print, spawn, uglifyjs, wrench, yaml, _,
  ...
  eb.Baker = (function() {
    ...
    Baker.prototype.runTest = function(command, args, options) {
      ...
      spawned = spawn((command === 'phantomjs' ? command : "node_modules/.bin/" + command), args);
      spawned.stdout.on('data', function(data) {
        return process.stderr.write(data.toString());
      });
      return spawned.on('exit', function(code) {
        var test_filename;
        test_filename = eb.Utils.removeString((command === 'phantomjs' ? args[1] : args[0]), "file://" + _this.YAML_dir + "/");
        if (code === 0) {
          if (!options.silent) {
        ...
        for (_i = 0, _len = file_groups.length; _i < _len; _i++) {
          ...
          for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
            ...
            if (set_options.command === 'phantomjs') {
              ...
              args.push(this.YAMLRelative(file));
            }
