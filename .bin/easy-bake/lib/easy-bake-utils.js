...
(function() {
  var KNOWN_SYSTEM_FILES, eb, fs, globber, path, _;
  ...
  eb.utils.extractSetBundles = function(set_options, queue, cwd) {
    var bundle_name, entries, _ref, _results;
    if (!set_options.bundles) {
      return;
    }
    _ref = set_options.bundles;
    _results = [];
    for (bundle_name in _ref) {
      entries = _ref[bundle_name];
      _results.push(queue.push(new eb.command.Bundle(bundle_name, entries, {
        cwd: cwd
      })));
    }
    return _results;
  ...
  eb.utils.resolveArguments = function(args, cwd) {
    return _.map(args, function(arg, index) {
      ...
      if (args[index - 1] === '-o' || args[index - 1] === '--output') {
        return eb.utils.resolvePath(arg, cwd, true);
      } else {
  ...
  eb.utils.resolvePath = function(target, cwd, skip_require) {
    var char, is_file, next_char, raw_target, _i, _len;
    is_file = target.search(/^file:\/\//) >= 0;
    if (is_file) {
      target = target.replace(/^file:\/\//, '');
    }
    if (!(skip_require || is_file)) {
